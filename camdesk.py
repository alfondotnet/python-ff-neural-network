...
# Esto es copiar pegar de internet
# Es un widget para que te aparezca la webcam en pantalla y poder hacerte las fotos
# Bien
...
class CamDesk(gtk.Window):
	...
	def take_snapshot(self,widget,event) :
		if event.keyval == gtk.keysyms.F3 :
			#self.movie_window.window.get_image(0, 0, 500, 400)
			filename = "temp.jpeg"
			img = gtk.gdk.Pixbuf( gtk.gdk.COLORSPACE_RGB, True, 8, 320, 240 )
			img.get_from_drawable(self.movie_window.window, self.movie_window.window.get_colormap(), 0, 0, 0, 0, 320, 240)
			img.save(filename, "jpeg", {"quality":"100"})
			md = gtk.MessageDialog(self,gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_INFO, gtk.BUTTONS_CLOSE, "Foto guarda skisa")
			md.run()
			md.destroy()
	...
	def __init__(self):
		...
		self.connect("key-press-event", self.take_snapshot)
		...
		# connect the signal "take_snapshot"
		#signal.signal(signal.SIGUSR1, self.take_snapshot_SIGUSR1)
	...
	def take_snapshot_SIGUSR1(signum, frame):
			filename = "temp.jpeg"
			img = gtk.gdk.Pixbuf( gtk.gdk.COLORSPACE_RGB, True, 8, 320, 240 )
			img.get_from_drawable(self.movie_window.window, self.movie_window.window.get_colormap(), 0, 0, 0, 0, 320, 240)
			img.save(filename, "jpeg", {"quality":"100"})
